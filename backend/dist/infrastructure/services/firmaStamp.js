...
exports.estamparFirmaDocx = estamparFirmaDocx;
/**
 ...
 */
function estamparFirmaDocx(docxBuffer, firmaBuffer) {
    const zip = new adm_zip_1.default(docxBuffer);
    let xml;
    try {
    ...
    if (!presentes.length)
        return docxBuffer; // sin bloques de firma → sin cambios
    ...
    if (zip.getEntry(mediaName))
        zip.updateFile(mediaName, firmaBuffer);
    else
        zip.addFile(mediaName, firmaBuffer);
    // 2) Relación imagen en document.xml.rels.
    const relsPath = 'word/_rels/document.xml.rels';
    const relId = 'rIdFirmaAI';
    let rels = '';
    try {
        rels = zip.readAsText(relsPath);
    }
    catch {
        return docxBuffer;
    }
    if (!rels.includes(`Id="${relId}"`)) {
        rels = rels.replace('</Relationships>', `<Relationship Id="${relId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/firma_ai.png"/></Relationships>`);
...
//# sourceMappingURL=firmaStamp.js.map