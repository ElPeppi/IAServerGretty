...
function nombreDeArchivo(file: string, cedula: string): string {
  const base = file.replace(/\.(docx|pdf)$/i, '').replace(/^BORRADOR\s+/i, '');
  const m =
    base.match(/CONTRA\s+(.+?)\s+(?:C\.?\s*C\.?\s*)?\d{5,}(?:\s+.*)?$/i) ||  // CONTRA {nombre} [C.C.] {cedula} [Y OTROS…]
...
async function main() {
  ...
  const lawyer =
    (await prisma.user.findFirst({ where: { role: 'LAWYER' } })) ??
  ...
  const existentes = new Set(
    ...
    ).map((d) => d.clientCedula as string)
  );
  ...
  for (const dir of folders) {
    ...
    const files = fs.readdirSync(path.join(DOCS_DIR, folder));
    ...
    const noTmp = (f: string) => !f.startsWith('~$');
    const esDocx = (f: string) =>
      /\.docx$/i.test(f) && noTmp(f) && /DEMANDA/i.test(f) && /(CONTRA|SINGULAR|EJECUT)/i.test(f);
    const esPdf = (f: string) =>
      /\.pdf$/i.test(f) && /DEMANDA/i.test(f) && /(CONTRA|SINGULAR|EJECUT)/i.test(f) && !/RADICAC/i.test(f);

    const demanda =
      files.find((f) => esDocx(f) && !/BORRADOR/i.test(f)) ||
      files.find((f) => esPdf(f)  && !/BORRADOR/i.test(f)) ||
      files.find((f) => esDocx(f)) ||
      files.find((f) => esPdf(f));
    if (!demanda) { saltados++; continue; }

    if (existentes.has(cedula)) { saltados++; continue; }
