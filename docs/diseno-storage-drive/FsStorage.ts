 ...
 *   backend/src/infrastructure/storage/FsStorage.ts
 *
 * Es básicamente el `NasStorage` actual adaptado a la interfaz `IStorage`
 * (métodos async + stream). DOCS_DIR = raíz (la MISMA carpeta que SAC_OUT_DIR del
 * motor). No copia a S3 ni al disco del servidor: la fuente es esa carpeta.
 */
import fs from 'fs';
